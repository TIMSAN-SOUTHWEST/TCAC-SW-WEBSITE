...
export const sendResetCode = createAsyncThunk(
  ...
  async ({ email }, thunkAPI) => {
    try {
      const response = await api.post('/auth/user/send-reset-code', { email });
...
export const verifyResetCodeAndChangePassword = createAsyncThunk(
  ...
  async ({ email, code, password }, thunkAPI) => {
    try {
      const response = await api.post('/auth/user/verify-reset-code', { email, code, password });
      return response.data;
    } catch (error) {
      const statusCode = error.response?.status || 500;
...
export const registerUser = createAsyncThunk(
  ...
  async (formValues, thunkAPI) => {
    try {
      const response = await api.post('/auth/user/register', formValues);
      return response.data;
    } catch (error) {
      const statusCode = error.response?.status || 500;
...
export const verifyUserEmail = createAsyncThunk(
  ...
  async (formValues, thunkAPI) => {
    try {
      const response = await api.post('/auth/user/verify-email', formValues);
      return response.data;
    } catch (error) {
      const statusCode = error.response?.status || 500;
...
export const resetUserPassword = createAsyncThunk(
  ...
  async (formValues, thunkAPI) => {
    try {
      const response = await api.post('/auth/user/reset-password', formValues);
      return response.data;
    } catch (error) {
      const statusCode = error.response?.status || 500;
...
const userAuthSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(registerUser.pending, (state) => { state.loading = true; state.status = 'pending'; })
      .addCase(registerUser.fulfilled, (state, action) => { state.loading = false; state.status = 'succeeded'; state.user = action.payload.user; })
      .addCase(registerUser.rejected, (state, action) => { state.loading = false; state.status = 'failed'; state.error = action.payload; })
      .addCase(loginUser.pending, (state) => { state.loading = true; state.status = 'pending'; })
      .addCase(loginUser.fulfilled, (state, action) => { state.loading = false; state.status = 'succeeded'; state.user = action.payload.user; state.token = action.payload.token; })
      .addCase(loginUser.rejected, (state, action) => { state.loading = false; state.status = 'failed'; state.error = action.payload; })
      .addCase(verifyUserEmail.pending, (state) => { state.loading = true; state.status = 'pending'; })
      .addCase(verifyUserEmail.fulfilled, (state) => { state.loading = false; state.status = 'succeeded'; })
      .addCase(verifyUserEmail.rejected, (state, action) => { state.loading = false; state.status = 'failed'; state.error = action.payload; })
      .addCase(resetUserPassword.pending, (state) => { state.loading = true; state.status = 'pending'; })
      .addCase(resetUserPassword.fulfilled, (state) => { state.loading = false; state.status = 'succeeded'; })
      .addCase(resetUserPassword.rejected, (state, action) => { state.loading = false; state.status = 'failed'; state.error = action.payload; })
