...
const UserDashboard = ({ accountData: initialData }) => {
  ...
  const printSlip = async () => {
    const res = await api.post('/slips', { userId: accountData._id });
    const data = res.data;
  ...
  const fetchPaymentHistory = useCallback(async (userId) => {
    if (!userId) return;
    try {
      const res = await api.get(`/payments/history?userId=${userId}`);
      const data = res.data;
  ...
  const refreshUserData = useCallback(async () => {
    if (!initialData?.id && !initialData?._id) return;
    const userId = initialData.id || initialData._id;
    const res = await api.get(`/users/${userId}`);
    const data = res.data;
    setAccountData(data);
    fetchPaymentHistory(data.id || data._id);
  }, [initialData, fetchPaymentHistory]);
  ...
  }, [initialData?.id, initialData?._id, refreshUserData]);
