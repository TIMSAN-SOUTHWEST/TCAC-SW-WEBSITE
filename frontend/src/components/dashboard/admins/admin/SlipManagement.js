...
const SlipManagement = () => {
  ...
  useEffect(() => {
    const fetchSlips = async () => {
      setLoading(true);
      try {
        const res = await api.get("/slips/admin");
        const data = res.data;
        setSlips(data.slips || []);
      } catch {
        setSlips([]);
  ...
  const handleViewSlip = async (slip) => {
    ...
    if (slip.user && slip.user._id) {
      try {
        const res = await api.get(`/payments/history?userId=${slip.user._id}`);
        const data = res.data;
  ...
  const printSlip = async () => {
    ...
    try {
      const res = await api.post("/slips", { userId: selectedSlip.user._id });
      const data = res.data;
