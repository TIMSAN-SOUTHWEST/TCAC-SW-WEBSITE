...
const PostManagement = () => {
  ...
  const handleDrop = async (e, targetPost) => {
    ...
    try {
      ...
      const response = await api.put("/posts/reorder", {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let response;
      if (editingPost) {
        response = await api.put(`/posts/${editingPost._id}`, dataToSend);
      } else {
        response = await api.post("/posts", dataToSend);
      }
  ...
  const handleDelete = async () => {
    try {
      const response = await api.delete(`/posts/${deletePost._id}`);
      const data = response.data;
