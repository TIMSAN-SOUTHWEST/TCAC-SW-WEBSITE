...
const AdminPaymentApprovalTable = () => {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      const url = `/payments/admin${params.toString() ? `?${params.toString()}` : ""}`;
  ...
  }, [fetchPayments, statusFilter, searchQuery]);
  ...
  const handleAction = async id => {
    ...
    try {
      const res = await api.post("/payments/update-status", { paymentId: id, status, adminComment, userId: payment.userId?._id || payment.userId, amount: payment.amount });
      const data = res.data;
      if (data.success) {
      ...
      toast({ title: "Error", description: error.response?.data?.error || "Failed to update payment", status: "error", duration: 4000, isClosable: true });
    }
